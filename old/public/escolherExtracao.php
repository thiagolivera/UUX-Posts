...
<div class="box box-solid">
    <div class="box-body" id="escolhaForma">
        ...
        <div class="list-group">
            <a href="#" class="list-group-item list-group-item-action" onclick="abrirEscolhaSite()">Extrair postagens do <b>Twitter</b></a>
            <a href="#" class="list-group-item list-group-item-action" onclick="abrirEnvioPlanilha()">Enviar um arquivo <b>CSV</b> com postagens</a>
            <a href="#" class="list-group-item list-group-item-action" onclick="abrirEnvioJSON()">Enviar um arquivo <b>JSON</b> com postagens</a>
        </div>
    </div>
    ...
    <div id="envioPlanilha" style="display: none">
        ...
        <div id="btnSalvar" style="float: right; padding-top: 10px; display: none">
            <button type="submit" class="btn btn-info" style="margin-right: 10px;">Enviar CSV</button>
        </div>
    ...
    <div id="envioJSON" style="display: none">
        <div class="box-body">
            <form action="public/extracaoJSON/controleExtracaoJSON.php" method="POST" enctype="multipart/form-data"> 
                <div class="form-group">
                    <h4 class="box-title" style="    padding-bottom: 10px;">Envie um arquivo com postagens em formato JSON</h4>
                    <div class="btn btn-default btn-file">
                        <input id="fileUpload" required="required" type="file" accept=".json" name="fileUpload">
                    </div> <br>
                </div>

                <div class="form-group" style="padding-left: 20px">
                    <br>
                    <strong class="row">Restrições do arquivo JSON</strong>
                    <ul style="text-align: left">
                        <li>Obrigatoriamente o arquivo deve conter o atributo "texto" contendo o conteúdo da postagem</li>
                        <li>Opcionalmente também pode existir o atributo "date" com  a data da postagem (caso não tenha esse atributo, a data será definida como null)</li>
                        <li>O arquivo deve seguir a formatação padrão de arquivos JSON</li>
                        <li><strong>Certifique-se de que cada postagem está em um índice de array no arquivo JSON</strong> </li>
                        <li>Caso o arquivo enviado não obedecer a tais restrições, ele pode ser lido incorretamente</li>
                        <li><a href="exemplo-json-uuxposts.json">Clique aqui e baixe um arquivo JSON de exemplo</a></li>
                    </ul>
                </div>
        </div>
        <div id="btnSalvarJSON" style="float: right; padding-top: 10px; display: none">
            <button type="submit" class="btn btn-info" style="margin-right: 10px;">Enviar JSON</button>
        </div>

        <div id="btnVoltarJSON" style="float: left; padding-top: 10px; display: none">
            <button type="button" class="btn btn-info" onclick="voltar()" style="margin-left: 10px;">Voltar</button>
        </div>
...
<script>
    ...
    function abrirEnvioPlanilha() {
        ...
        document.getElementById('btnVoltarJSON').style.display = 'none';
        document.getElementById('btnSalvarJSON').style.display = 'none';
        document.getElementById('escolhaForma').style.display = 'none';
        document.getElementById('btnVoltarExt').style.display = 'none';
    }

    function abrirEnvioJSON() {
        document.getElementById('envioJSON').style.display = 'block';
        document.getElementById('btnVoltarJSON').style.display = 'block';
        document.getElementById('btnSalvarJSON').style.display = 'block';
    ...
    function voltar() {
        ...
        document.getElementById('envioPlanilha').style.display = 'none';
        document.getElementById('envioJSON').style.display = 'none';
        document.getElementById('btnVoltar').style.display = 'none';
        document.getElementById('btnSalvar').style.display = 'none';
        document.getElementById('btnVoltarJSON').style.display = 'none';
        document.getElementById('btnSalvarJSON').style.display = 'none';
